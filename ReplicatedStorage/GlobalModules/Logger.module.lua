...
local logger = {}
logger._SkipRegistry = setmetatable({}, { __mode = "k" })
...
-- ENTRY POINT
function logger:Skip(fn)
    self._SkipRegistry[fn] = true
    return fn
end
function logger:WrapModule(moduleTable, moduleName)
	for k, v in pairs(moduleTable) do
		if type(v) == "function" and not self._SkipRegistry[v] and RunService:IsStudio() then
